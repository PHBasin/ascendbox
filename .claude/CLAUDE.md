...
The contract's rules live in **`scripts/lib/exercise-contract.ts`** (`validateExercises()` — pure, no I/O, no `process.exit`); `validate-data.ts` is only the file-reading, exit-code half of the CI gate. The split exists because the **CSV round trip** (`scripts/exercises-csv.ts`, `npm run data:export` / `data:import` / `data:verify`) has to apply the _same_ rules to a payload that is not on disk yet — a spreadsheet is how a coach would rather author 115 exercises, and a second, drifting transcription of the contract would be worse than no check at all. Import therefore validates **before writing**: a bad CSV leaves `exercises.json` untouched. The CSV dialect is `;`-separated (RFC 4180 quoting, CRLF, UTF-8 BOM — what a French Excel opens without an import wizard), and list fields collapse to one `|`-separated cell rather than to numbered columns, since `instructions` has no fixed length; export refuses to write a value that already contains `|`. `FIELD_KINDS` there is the same mapped type over `keyof Required<Exercise>`, so a new field fails `type-check` in **both** files until it is given a rule _and_ a column — a round trip can never silently drop one. `data:verify` is the fidelity proof standing in for the test runner this repo does not have.
