...
- `npm run data:import` — CSV → JSON (`-- --in fichier.csv`; `--dry-run` validates without writing)
- `npm run data:verify` — JSON → CSV → JSON in memory, diffed: proves the round trip is lossless
...
A spreadsheet is how a coach would rather author 115 exercises, so **`scripts/exercises-csv.ts`** (`npm run data:export` / `data:import` / `data:verify`) converts both ways — and contains **no validation of its own**. It spawns `node scripts/validate-data.ts <fichier>` and obeys the exit code: the same gate, invoked, not a second transcription of the rules that could drift from them. The validator takes an optional path for exactly this reason; with no argument it is the CI gate, unchanged. On import the JSON is staged to `<cible>.staging-<pid>` beside the target, validated _there_, then moved in with an atomic rename — so a rejected CSV leaves `exercises.json` byte-for-byte untouched, with no half-written window and no roll-back to get wrong. The CSV dialect is `;`-separated (RFC 4180 quoting, CRLF, UTF-8 BOM — what a French Excel opens without an import wizard), and list fields collapse to one `|`-separated cell rather than to numbered columns, since `instructions` has no fixed length; export refuses to write a value that already contains `|`. `FIELD_KINDS` there is the same mapped type over `keyof Required<Exercise>` that `FIELDS` is, so a new field fails `type-check` in **both** files until it is given a rule _and_ a column — a round trip can never silently drop one. `data:verify` re-encodes the catalogue and diffs it back: the fidelity proof standing in for the test runner this repo does not have.
