...
- `npm run data:import` — CSV → JSON (`-- --in fichier.csv`; `--dry-run` validates without writing). Both legs run `validate:data`; there is no separate verification command, by design
...
A spreadsheet is how a coach would rather author 115 exercises, so the **CSV round trip** (`npm run data:export` / `data:import`) converts both ways. It is three files, one responsibility each — the split that lets the mapping be unit-tested the day a runner lands:

- **`scripts/lib/exercise-csv.ts`** — the `Exercise` ⇄ CSV mapping. **Pure**: no I/O, no `process`, no `console`. Encoding and decoding sit together because they are one contract read in two directions. `csv-parse` / `csv-stringify` (devDeps) do the parsing and quoting; the file only maps fields to cells. `FIELD_KINDS` is the same mapped type over `keyof Required<Exercise>` that `FIELDS` is, so a new field fails `type-check` in **both** files until it has a rule _and_ a column — a round trip can never silently drop one.
- **`scripts/lib/catalogue-store.ts`** — the tooling-side sibling of `src/data/exerciseRepository.ts`: every file and process touch. Throws on I/O faults, returns a verdict on contract faults, formats neither.
- **`scripts/exercises-csv.ts`** — the CLI. `node:util`'s native `parseArgs` (a typo'd flag is an error, not a silent default), dispatch, messages, exit codes.

**No validation is restated or refactored anywhere in them**: the store spawns `node scripts/validate-data.ts <fichier>` and obeys the exit code — the same gate, invoked. The validator takes an optional path for exactly this reason; with no argument it is the CI gate, unchanged. On import the candidate JSON is staged to `<cible>.staging-<pid>` beside the target, validated _there_, then moved in with an atomic rename (`try`/`finally` clears the staging file on every path) — so a rejected CSV leaves `exercises.json` byte-for-byte untouched, with no half-written window and no roll-back to get wrong. An import carrying **no data row is refused** rather than obeyed: an empty array passes the contract, so honouring it would wipe the catalogue and report success.

The dialect is `;`-separated (RFC 4180 quoting, CRLF, UTF-8 BOM — what a French Excel opens without an import wizard); list fields collapse to one `|`-separated cell rather than to numbered columns, since `instructions` has no fixed length, and export refuses to write a value that already contains `|`. There is deliberately **no round-trip verification command**: `validate:data` is the check, run on both legs, and a second one would only be a second thing to keep in step.
