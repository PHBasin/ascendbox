...
A spreadsheet is how a coach would rather author 115 exercises, so the **CSV round trip** (`npm run data:export` / `data:import`) converts both ways. Two files, split along the only line that earns a file — pure logic versus side effects:
...
- **`scripts/exercises-csv.ts`** — everything with a side effect: file reads and writes, `node:util`'s native `parseArgs` (a typo'd flag is an error, not a silent fallback to the default path), messages, exit codes. Running a command _is_ the side effect, so this is where they belong — a separate I/O module was tried and deleted, being four one-line wrappers around `readFileSync` once the converter stopped orchestrating anything.

**They convert, and nothing else.** Checking the content is `npm run validate:data` — the existing gate, neither wrapped nor restated nor modified (it is byte-identical to what it was before this tooling existed) — run as a second step after an import. Two commands, one job each, rather than one command with an opinion about when the other should fire. The cost is real and deliberate: an import writes whatever the CSV says, so a bad one lands in `exercises.json` and it is `validate:data` that catches it, after the fact. `git checkout` is the undo.
