...
The data file (`public/data/exercises.json`) is a **bare array**; each entry is `{ id, title, teaser, categoryId, tags, level, duration }` (`level` is `1 | 2 | 3`, `duration` in minutes) plus the **optional** detail fields `{ objective?, equipment?, instructions?: string[], variants?: { harder?, easier? }, safety? }` (§5.6). **~150 exercises** over a vocabulary of **16 tags**, split unevenly - technique holds roughly three fifths of the catalogue, physique and mental about a fifth each. The size stays an approximation on purpose: it moves with every authoring session, and `npm run validate:data` prints the exact figure on every run. The tag count is exact because that vocabulary changes rarely and deliberately.
