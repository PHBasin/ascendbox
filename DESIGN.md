...
> **Contexte technique** : Tailwind CSS **v4** (moteur `tailwindcss@next`), config _CSS-first_
...
l'espacement en `@theme` — contrairement aux couleurs, l'échelle numérique de Tailwind _est_
...
4. **Entre cartes / conteneur** (`xl`) — gouttière du feed _et_ marge du conteneur partagent `24px`,
...
- **Rester sur les pas ci-dessus.** Pas de valeur arbitraire (`px-5`, `px-2.5`, `p-[13px]`…) :
  choisir le pas nommé le plus proche. _(Régularisé : les filtres passent de `px-5`→`px-4`,
  les chips de `px-2.5`→`px-3`.)_
