...
- **`src/domain/exercise.ts`** — pure entities/types, no framework or data deps. Defines the `Exercise` interface and the **single source of truth for categories**: `PILIERS` (`['Force', 'Technique', 'Mental']` as const) and the derived `Pilier` type. Add new categories here only.
- **`src/data/exerciseRepository.ts`** — the ONLY module that knows the data source. It **`fetch`es `public/data/exercises.json` at runtime** (kept out of the JS bundle for TTI; preloaded via `<link rel="preload">` in `index.html`), `Object.freeze`s the result, and caches it. `getAllExercises()` is `async`. Swap this file to move to an API; nothing upstream changes.
...
- **`src/components/`** — presentational only. `App.vue` composes `AscendBoxLogo`, `CategoryFilter` (emits `select` with a `Pilier`), and `ExerciseFeed` (renders `ExerciseCard`s in a `TransitionGroup`).
- **Path alias**: `@` → `./src` (declared in both `vite.config.ts` and `tsconfig.json` — keep them in sync).

The data file (`public/data/exercises.json`) is a **bare array**; each entry is `{ title, description, categoryId, tags, intensity, duration }` and has **no `id`** (the repository assigns one). The `Exercise` interface (`src/domain/exercise.ts`) adds `id` and mirrors the rest. Categories are lowercase ids (`force`/`technique`/`mental`) that intentionally match the `--color-*` tokens; display labels come from the `CATEGORIES` array. Since the JSON is fetched (not imported), a schema drift won't be caught by `vue-tsc` — it fails at runtime, so keep the file aligned with the interface manually.
